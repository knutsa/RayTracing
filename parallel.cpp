...
vector<Sphere> generate_common_scene(int n_small_balls = 100, double small_r = 0.1)
{
    ...
    if (rank == 0)
    {
        ...
        scene_params.first.resize((n_small_balls + 3) * max_material_param_size()); // MPI On dardel demands all sizes are exactly equal during bcast
    }
    else
    {
...
void generate_image_fixed(const Camera &camera, const vector<Sphere> &scene, int samples_per_pixel, string fname = "image_fixed")
{
    int P, rank;
    MPI_Comm_size(MPI_COMM_WORLD, &P);
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    int my_samples = (int) ceil(samples_per_pixel/(double) P);

    auto start = high_resolution_clock::now();
    auto img = camera.render(scene, my_samples);
...
double timing_seconds_fixed(const Camera &camera, const vector<Sphere> &scene, int samples_per_pixel, int num_runs = 10)
{
    int P, rank;
    MPI_Comm_size(MPI_COMM_WORLD, &P);
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
